...
const SkillTree: React.FC<SkillTreeProps> = ({ gameState, onBuySkill }) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            if (ref.current) {
                ...
                // Scroll to center of the 2000x2000 canvas
                // Canvas center is 1000,1000. Viewport center should match.
    ...
    return (
        <div className="relative w-full h-full bg-slate-900 overflow-hidden flex flex-col">
            ...
            >
                <div className="relative w-[2000px] h-[2000px] bg-slate-950/50">
                    <div className="absolute inset-0 pointer-events-none" style={{
                        backgroundImage: 'radial-gradient(circle at 1px 1px, rgba(255,255,255,0.05) 1px, transparent 0)',
                        backgroundSize: '40px 40px'
                    }}></div>

                    {/* Content Container - Centered at 1000, 1000 */}
                        ...
                        {SKILL_TREE.map(node => {
                            ...
                            return (
                                <div key={node.id} className="absolute" style={{ left: node.x, top: node.y }}>
                                    ...
                                    {/* Tooltip */}
                                    {isHovered && (
                                        <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-4 w-64 bg-slate-900/95 border border-amber-900/50 p-3 rounded-lg shadow-2xl backdrop-blur-md text-slate-100 z-50 pointer-events-none">
